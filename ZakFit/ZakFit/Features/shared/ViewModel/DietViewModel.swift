...
class DietViewModel{
            ...
            if let data = data {
                do{
                    ...
                    DispatchQueue.main.async {
                        ...
                        self.dietsFetch = self.diets.map{
                            DietEnum(rawValue: $0.name) ?? .flexitarien
                        }
                    }
                }
                catch {
    ...
    func deleteDiet(obj : Diet){
        ...
        guard let url = URL(string: "http://localhost:8080/diet/\(obj.id)")
        else {
            print("mauvais url")
            return
        }
